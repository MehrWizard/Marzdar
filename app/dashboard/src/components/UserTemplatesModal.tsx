...
import { UserTemplate, UserTemplateCreate } from "types/UserTemplate";
import {
...
export const UserTemplatesModal: FC = () => {
  ...
  const { createTemplate, modifyTemplate, deleteTemplate } = useUserTemplates();
  ...
  const availableProtocols: [string, { tag: string }[]][] = useMemo(() => {
    if (!systemInbounds) return [];
    if (systemInbounds instanceof Map) {
      return Array.from(systemInbounds.entries());
    }
    if (typeof systemInbounds === "object") {
      return Object.entries(systemInbounds) as [string, { tag: string }[]][];
    }
    return [];
  }, [systemInbounds]);
    ...
    try {
      if (systemInbounds instanceof Map) {
        systemInbounds.forEach((items, proto) => {
          allIns[proto] = (items || []).map((i: any) => i.tag);
        });
      } else if (systemInbounds && typeof systemInbounds === "object") {
        Object.entries(systemInbounds).forEach(([proto, items]: [string, any]) => {
          allIns[proto] = (items || []).map((i: any) => i.tag);
        });
      }
    } catch (e) {
      console.warn("Could not pre-populate inbounds:", e);
    }
            ...
            {isLoading ? (
              ...
              !isFormOpen && (
                <Box textAlign="center" py={8} color="gray.500">
                  <Text fontSize="sm" mb={3}>{t("templates.noTemplates")}</Text>
                  <Button
                    size="sm"
                    colorScheme="primary"
                    variant="outline"
